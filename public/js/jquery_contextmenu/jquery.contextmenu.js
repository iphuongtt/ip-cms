(function($) {
    ...
    $.fn.contextmenu = function(option) {
		...
		itemTpl = "<div class='b-m-$[type]' unselectable=on><nobr unselectable=on><span class ='list_img $[icon]'></span><span unselectable=on class = 'item-context'>$[text]</span></nobr></div>";
        ...
        var me = $(this).each(function() {
            return $(this).bind('contextmenu', function(e, e1) {
                var gete = e;
                if(e1)
                    gete = e1;
                var bShowContext = (option.onContextMenu && $.isFunction(option.onContextMenu)) ? option.onContextMenu.call(this, gete) : true;
                if (bShowContext) {
                    if (option.onShow && $.isFunction(option.onShow)) {
                        option.onShow.call(this, root);
                    }
                    root.showMenu(gete, this);
                }
                return false;
            });
        });
