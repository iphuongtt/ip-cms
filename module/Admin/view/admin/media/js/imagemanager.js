	...
	window.ImageManager = $.extend(BaseManager, {
		...
		listFiles : function(p) {
			...
			RPC.exec('im.listFiles', {
				...
				if (!RPC.handleError({message : 'List files error', visual_path : t.visualPath, response : data})) {
					...
					$.each(t.files, function(index, value){
						if(value.type != 'FLV' && value.type != 'Flv' && value.type != 'flv')
							t.files[index]['myPath'] = '../../files' + value.path.replace('{0}','');
						else
							t.files[index]['myPath'] = 'video.php?id=' + value.path.replace('{0}/','');
					});
					// Update file list
					t.updateFileList();
		...
		uploadFiles : function() {
			$.WindowManager.open({
				...
				path : this.path,
				visual_path : this.visualPath,
				onupload : function(aaaa) {
					test = aaaa;
					$.each(test.files, function(){
						alert(this.name);
					});
					ImageManager.listFiles();
				}
